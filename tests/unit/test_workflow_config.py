...
# --------------------------------------------------------------------------- #
# T-220 (REQ-CM-003) — caveman_mode capability toggle (default OFF, strict `is True`)
# + caveman_level enum (`lite`|`full`, default `lite`, invalid → `lite`); fail-soft.
# --------------------------------------------------------------------------- #


def test_caveman_mode_defaults_off(tmp_path):
    forge = _write_config(tmp_path, "orchestration:\n  parallel_build: true\n")
    cfg = _wfc.load_orchestration_config(forge)
    assert cfg.caveman_mode is False


def test_caveman_mode_defaults_off_when_no_config(tmp_path):
    cfg = _wfc.load_orchestration_config(tmp_path / ".forge")
    assert cfg.caveman_mode is False


def test_caveman_mode_true_enables(tmp_path):
    forge = _write_config(tmp_path, "orchestration:\n  caveman_mode: true\n")
    cfg = _wfc.load_orchestration_config(forge)
    assert cfg.caveman_mode is True


def test_caveman_mode_truthy_nonbool_does_not_enable(tmp_path):
    # Strict `is True`: a stray `1` / `"yes"` never silently enables the capability.
    for stray in ("1", '"yes"'):
        forge = _write_config(tmp_path, f"orchestration:\n  caveman_mode: {stray}\n")
        cfg = _wfc.load_orchestration_config(forge)
...
def test_caveman_mode_in_toggles_tuple(tmp_path):
    # caveman_mode is a real capability toggle — it must live in `_TOGGLES` so the strict
    # `is True` parse loop covers it (not a bespoke branch).
    assert "caveman_mode" in _wfc._TOGGLES


def test_caveman_level_defaults_lite(tmp_path):
    cfg = _wfc.load_orchestration_config(tmp_path / ".forge")
    assert cfg.caveman_level == "lite"


def test_caveman_level_full_accepted(tmp_path):
    forge = _write_config(tmp_path, "orchestration:\n  caveman_level: full\n")
    cfg = _wfc.load_orchestration_config(forge)
    assert cfg.caveman_level == "full"


def test_caveman_level_lite_accepted(tmp_path):
    forge = _write_config(tmp_path, "orchestration:\n  caveman_level: lite\n")
    cfg = _wfc.load_orchestration_config(forge)
    assert cfg.caveman_level == "lite"


def test_caveman_level_invalid_coerces_to_lite(tmp_path):
    # An unknown intensity (`ultra`/`wenyan`/garbage/non-str) coerces to the safe default.
    for bad in ("ultra", "wenyan", "LITE", "5", "[]"):
        forge = _write_config(tmp_path, f"orchestration:\n  caveman_level: {bad}\n")
        cfg = _wfc.load_orchestration_config(forge)
        assert cfg.caveman_level == "lite", f"{bad!r} should coerce to lite"


def test_caveman_mode_independent_of_other_toggles(tmp_path):
    # Enabling caveman_mode alone leaves every other capability toggle off.
    forge = _write_config(tmp_path, "orchestration:\n  caveman_mode: true\n")
    cfg = _wfc.load_orchestration_config(forge)
    assert cfg.caveman_mode is True
    for other in TOGGLES:
