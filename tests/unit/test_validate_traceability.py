"""Tests for scripts/validate-traceability.py (root Claude Code plugin).
...
rollup of the pre-existing traceability/gate scripts. Mirrors
test_opencode_validate_traceability.py; see _trace_scan.py for the shared
implementation both the root and forge-opencode/ copies import.
"""
...
SCRIPT = _ROOT / "scripts" / "validate-traceability.py"
PYTHON = sys.executable

# Hyphenated filename — must use importlib
_spec = importlib.util.spec_from_file_location("root_validate_traceability", SCRIPT)
_mod = importlib.util.module_from_spec(_spec)
sys.modules["root_validate_traceability"] = _mod
...
class TestReportAndCLI:
    ...
    def test_out_flag_writes_file_not_stdout(self, tmp_path):
        ...
        result = subprocess.run(
            [PYTHON, str(SCRIPT), "--cwd", str(tmp_path), "--plugin-dir", str(_ROOT), "--out", str(out)],
            capture_output=True,
            text=True,
        )
    ...
    def test_report_includes_gate_rollup_section(self, tmp_path):
        _write(tmp_path / "pipeline" / "01-srs" / "srs.md", "### REQ-001\nSomething.\n")
        issues, gates = build_report(tmp_path, _ROOT)
