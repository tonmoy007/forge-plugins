...
# --- caveman mode (T-222 — REQ-CM-002 / NF-041) -----------------------------
# The chokepoint prepends a terse-output preamble to FREE-PROSE prompts when a caveman level is
# resolved (threaded `caveman` arg, else the FORGE_CAVEMAN env, else off), and NEVER to a
# schema-constrained dispatch. "Answer with maximum brevity" is the preamble's discriminator.
_PREAMBLE_MARK = "Answer with maximum brevity"


def test_dispatch_applies_caveman_preamble_for_free_prose(tmp_path, monkeypatch) -> None:
    monkeypatch.delenv("FORGE_CAVEMAN", raising=False)
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    _ba.dispatch("Summarise the run.", forge_dir=forge, feature="t",
                 caveman="lite", claude_bin=bin_)
    argv = argv_log.read_text()
    assert _PREAMBLE_MARK in argv
    assert "Summarise the run." in argv


def test_dispatch_skips_caveman_when_schema_set(tmp_path, monkeypatch) -> None:
    # Schema-constrained dispatch is byte-identical — the preamble is never prepended (REQ-NF-041).
    monkeypatch.delenv("FORGE_CAVEMAN", raising=False)
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    schema = {"type": "object", "properties": {"x": {"type": "string"}},
              "required": ["x"], "additionalProperties": False}
    _ba.dispatch("Return ONLY JSON.", forge_dir=forge, feature="t",
                 caveman="lite", output_schema=schema, claude_bin=bin_)
    assert _PREAMBLE_MARK not in argv_log.read_text()


def test_dispatch_no_caveman_by_default(tmp_path, monkeypatch) -> None:
    # Off (no arg, no env) ⇒ byte-identical to v0.6.0: no preamble.
    monkeypatch.delenv("FORGE_CAVEMAN", raising=False)
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    _ba.dispatch("Summarise the run.", forge_dir=forge, feature="t", claude_bin=bin_)
    assert _PREAMBLE_MARK not in argv_log.read_text()


def test_dispatch_caveman_env_fallback(tmp_path, monkeypatch) -> None:
    # With no threaded arg, FORGE_CAVEMAN supplies the level (global switch / test hook).
    monkeypatch.setenv("FORGE_CAVEMAN", "lite")
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    _ba.dispatch("Summarise the run.", forge_dir=forge, feature="t", claude_bin=bin_)
    assert _PREAMBLE_MARK in argv_log.read_text()


def test_dispatch_caveman_arg_overrides_env(tmp_path, monkeypatch) -> None:
    # The threaded arg wins over the env; `full` selects the fragment-dropping preamble.
    monkeypatch.setenv("FORGE_CAVEMAN", "lite")
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    _ba.dispatch("Summarise the run.", forge_dir=forge, feature="t",
                 caveman="full", claude_bin=bin_)
    argv = argv_log.read_text()
    assert _PREAMBLE_MARK in argv
    assert "prefer fragments" in argv  # the `full`-only clause


def test_dispatch_caveman_invalid_level_is_off(tmp_path, monkeypatch) -> None:
    # An unsupported level (env `ultra`/`wenyan`/garbage) is treated as OFF at the chokepoint.
    monkeypatch.setenv("FORGE_CAVEMAN", "ultra")
    bin_, argv_log = _fake_dispatch_claude(tmp_path)
    forge = tmp_path / ".forge"
    _ba.dispatch("Summarise the run.", forge_dir=forge, feature="t", claude_bin=bin_)
    assert _PREAMBLE_MARK not in argv_log.read_text()
