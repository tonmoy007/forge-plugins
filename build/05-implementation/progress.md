...
- **NEXT**: **v0.4.1 PLANNED — "operable engine" hardening.** SRS
  ...
  one standing-non-goals list. **T-202..T-205 done; T-206 merged to `main` (manifests 0.4.1, CHANGELOG
  `[0.4.1]`, `2809e7e`) — but the `v0.4.1` tag + GitHub release are BLOCKED by an env-level proxy 403 on
  tag-ref pushes (see the T-206 row). Finish the tag/release from an env that permits tag pushes.**
...
| T-206 | 🟡 merged-to-main, **tag BLOCKED** | Release v0.4.1 — `bump-version.py 0.4.1` (manifests **0.4.1** on `main`), CHANGELOG `[0.4.1]`, ROADMAP/progress rows. Banner/social-preview evergreen (no version stats since `d0e04e6`) → no refresh. Pre-release green (1660 pass, validate 0, full-pipeline 12/12 toggles off+on). **Shipped via PR #36 (feat→develop, merge-resolved an older social-preview re-render on develop in favor of main's evergreen) + PR #37 (develop→main, `2809e7e`).** **BLOCKER: cannot push `refs/tags/v0.4.1` — the git proxy returns HTTP 403 on ALL tag-ref pushes (annotated AND lightweight); branch pushes work. No `create_release`/create-tag MCP tool exists either.** So the tag `v0.4.1` + GitHub release are NOT yet on origin; polygon remote is not configured in this env. **To finish: push `git tag v0.4.1 2809e7e && git push origin v0.4.1` from an environment whose proxy permits tag pushes (or create the tag/release in the GitHub UI), then publish the release.** Next routine run: main is 0.4.1 but the tag is absent, so the idempotency guard correctly re-enters at T-206 and will retry the tag/release. |
