...
| v0.3.5 | ⚪ planned | **Semantic skill mining + skill-creator** — replace the tool-name-n-gram miner with a semantic, success-gated, **anti-unification**-based miner that proposes genuine reusable *workflows*, authors them via a new `/forge:skill-creator` (agentskills.io `SKILL.md`), verifies by replay, and curates the library (ExpeL voting + `/dream`-style maintenance). Human-approved throughout. SRS `build/01-srs/srs-v0.3.5.md`, DAG `build/04-plan/task-dag-v0.3.5.md` (T-177–T-184). |
