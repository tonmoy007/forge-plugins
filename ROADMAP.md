...
| v0.3.6 | ⚪ planned | **Context-aware autopilot** — when an autopilot run crosses a configurable context threshold, automatically **checkpoint → compact → continue**. Background dispatches rotate the session on a real token-pressure signal; in-session runs ride native auto-compaction via a new **PreCompact** checkpoint + **SessionStart(`compact`)** resume injection, with a shared schema-versioned checkpoint and run-log idempotency. Opt-in (default 80%). SRS `build/01-srs/srs-v0.3.6.md`, DAG `build/04-plan/task-dag-v0.3.6.md` (T-185–T-190). |
