...
| ~~v0.7.0~~ | ⚪ folded | **Docker workflow enforcement + extensible tooling preflight** (fail-soft, never-block) — a declarative tool registry (`references/tool-registry.md`) + `scripts/tool_preflight.py` (detect via `shutil.which` + version probe, cached like the capability probe), an advisory `scripts/check_docker_readiness.py` wired **unconditionally** into `/forge:deploy` (cross-cutting — every project gets it, regardless of profile), the sole confirm-then-install surface `/forge:preflight`, and an opt-in `docker` profile (never auto-assigned over a real app type). ADR-012. Built as T-227–T-233 but **folded into the v0.8.0 release** alongside Builder Pro rather than tagged separately. SRS `build/01-srs/srs-v0.7.0.md`, DAG `build/04-plan/task-dag-v0.7.0.md`. |
| **v0.8.0** | 🟢 released | **Builder Pro (Stage 6 Pro tier) + Docker workflow enforcement — combined release.** `/forge:build-pro`: `scripts/build_executor.py` (deterministic context-resolve/four-check-gate/commit/traceability/build-log engine) + `agents/builder-pro.md` (thin generative router) + `skills/forge-build-pro/SKILL.md`; classic `/forge:build` unchanged, both tiers coexist. Configurable context-resolution depth (`spec_plan`/`spec_arch_plan`/`full_chain`, REQ-BUILDCTX-002) set once via a `/forge:plan-pro` Stage 5 pre-flight prompt. Ships together with the v0.7.0 Docker/tooling layer above (folded, not separately tagged) — same sitting, no shared files. ADR-012. Tag `v0.8.0`, both remotes. SRS `build/01-srs/srs-v0.8.0.md`, DAG `build/04-plan/task-dag-v0.8.0.md` (T-241–T-253, T-254 release). |
