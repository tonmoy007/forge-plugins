...
3. Read `pipeline/08-deploy/deploy-plan.md` and `pipeline/01-srs/srs.md` NFR section.
...
5. Write `pipeline/09-monitor/observability.md` and `pipeline/09-monitor/health-report.md`.
6. Run `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/state-manager.py advance --to 9` to mark Stage 9 active.

## Verification

After running, confirm:
- `pipeline/09-monitor/observability.md` exists with SLIs, SLOs, and alert thresholds
- `pipeline/09-monitor/health-report.md` exists with current status
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/state-manager.py next-hint --stage 9
```
